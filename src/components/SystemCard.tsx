...
export interface SystemCardProps {
  ...
  to: "/velocity" | "/strength" | "/nutrition" | "/recovery";
}

export function SystemCard({ index, title, tagline, description, subcategories, to }: SystemCardProps) {
  return (
    <Link
      to={to}
      className="group relative flex flex-col overflow-hidden rounded-2xl border border-border bg-card p-7 card-hover hover:[--tw:_] hover:-translate-y-1 hover:border-accent hover:shadow-[0_20px_60px_-20px_var(--accent-glow)]"
    >
      ...
      <div className="mt-7 flex items-center justify-between border-t border-border/60 pt-5">
        <span className="text-sm font-medium text-foreground">Explore System</span>
